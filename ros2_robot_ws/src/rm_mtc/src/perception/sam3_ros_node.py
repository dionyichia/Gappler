...
class Sam3RosNode(Node):
    def __init__(self):
        ...
        # Subscriber
        self.rgb_sub = self.create_subscription(Image, TOPIC_RGB, self.rgb_callback, 10)
    ...
    def rgb_callback(self, msg: Image):
        ...
        best_mask, best_score = ObjectMaskVisualizer.get_best_mask(results)

        if best_mask is None:
            self.get_logger().debug("No detection above threshold")
            return
        ...
        mask_msg.data = bytes(best_mask.flatten())

        self.mask_pub.publish(mask_msg)
        self.get_logger().info(f"Mask published with score: {best_score}")
