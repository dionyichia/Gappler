...
def save_audio(filepath):
    audio_streamer = AriaStreamClient()
    data_channels = [aria.StreamingDataType.Audio]
    message_size = 1000
    observer = audio_streamer.subscribe(data_channels, AudioObserver(), message_size)
    ...
    print(
        "Please speak 10 sentences, each lasting 6 seconds. Recording starts automatically."
    )
...
def benchmark_whisper(filepath):
    ...
    for model_name in whisper_models:
        ...
        for folder in audio_samples:
            ...
            wav_files = [
                os.path.join(
                    filepath, "data/experiments/audio", folder, f"sentence_{i+1}.wav"
                )
                for i in range(20)
            ]
            if len(wav_files) < 20:
            ...
            for n in range(10):  # 10 iterations
                ...
                for i, (audio_path, ref) in enumerate(
                    zip(wav_files, test_sentences * 2)
                ):
                    start = time.time()
            ...
            log_lines.append(
                f"CER ({folder}): mean={mean_cer:.3f}, std={std_cer:.3f}\n"
            )
        ...
        out_file = os.path.join(
            filepath, "data/experiments/results", f"{model_name}_results.txt"
        )
...
def main():
    ...
    if args.record_audio:
        ...
        aria = AriaDeviceManager()
        aria.connect_and_start_streaming(None, "usb", "profile18")
        # save 10 audio files
        save_audio(filepath)
