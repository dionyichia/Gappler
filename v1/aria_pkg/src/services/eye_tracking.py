...
from models.aria_eyetracking.projectaria_eyetracking.inference import infer
...
class EyeTrackingSystem:
    """Container for eye-tracking system components."""

    inference_model: infer.EyeGazeInference
    device_calibration: any
    rgb_stream_label: str
    rgb_camera_calibration: any
    rgb_linear_camera_calibration: any
...
def initialize_eye_tracking(device: str = "cuda") -> EyeTrackingSystem:
    """
    ...
    """
    # Perform calibration with initialization VRS file
    init_vrs_path = PROJECT_ROOT / "assets/aria/init.vrs"
    if not init_vrs_path.exists():
        raise FileNotFoundError(f"Calibration VRS file not found: {init_vrs_path}")

    provider = data_provider.create_vrs_data_provider(str(init_vrs_path))

    # Get RGB camera calibration
    rgb_stream_id = StreamId(RGB_STREAM_ID)
    ...
    # Create linearized camera calibration for undistortion
    ...
    return EyeTrackingSystem(
        ...
        rgb_linear_camera_calibration=rgb_linear_camera_calibration,
    )
